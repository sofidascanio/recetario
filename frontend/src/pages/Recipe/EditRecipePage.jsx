...
export default function EditRecipePage() {
    ...
    const [recipe, setRecipe] = useState(null) // datos de la receta
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState(null)

    // carga la receta una sola vez al montar
    useEffect(() => {
        recipesService.getById(id)
            .then(data => {
                // verifica que el usuario logueado sea el autor
...
// form (se monta solo cuando recipe está disponible)
// separado de EditRecipePage para que useRecipeForm reciba initialData en el primer render y no tenga que sincronizarse despues
function EditForm({ recipe, recipeId }) {
    const navigate = useNavigate()
    const form = useRecipeForm(recipe) // initialData pre-popula el estado
...
function StepTwo({ form }) {
    ...
    async function handleSearch(val) {
        setQuery(val)
        setSelected(null)
        if (val.length < 2) { setResults([]); setDropdownOpen(false); return }
        try {
            const data = await api.get('/ingredients', { params: { search: val } })
            setResults(data)
    ...
    async function handleCreateAndSelect() {
        ...
        try {
            const newIng = await api.post('/ingredients', { name: query.trim() })
            handleSelect(newIng)
        } catch { /* ignorar */ }
    }
            ...
            <ol className={styles.stepsList}>
                {form.data.steps.map((s, i) => (
                    <li key={i} className={styles.stepItem}>
                        ...
                        <Field label="Título del paso *" error={form.errors[`step_title_${i}`]}>
                            <input value={s.title}
                                   onChange={e => form.updateStep(i, 'title', e.target.value)}
                                   placeholder="Ej: Preparar la masa" />
                        </Field>

                        <Field label="Descripción *" error={form.errors[`step_desc_${i}`]}>
